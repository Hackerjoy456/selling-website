...
import { Copy, Check, CreditCard, Globe, MessageCircle, ChevronDown, ChevronUp, Image as ImageIcon } from "lucide-react";
  ...
  const paymentMethods: PaymentMethod[] = [
    {
      ...
      methods: [
        { label: "bKash", value: "01301571890", type: "number", image: "/assets/bkash.png" },
        { label: "Nagad", value: "01629933030", type: "number", image: "/assets/nagad.png" },
      ],
    },
    {
      id: "india",
      ...
      methods: [
        { label: "UPI ID", value: "airtel1397@ybl", type: "text", image: "/assets/upi.jpg" },
      ],
    },
    {
      id: "international",
      ...
      methods: [
        { label: "Binance ID", value: "912227968", type: "number", image: "/assets/binanace.png" },
        { label: "Username", value: "BakiHanma49", type: "username", image: "/assets/binanace.png" },
      ],
    },
  ];
          ...
          >
            ...
            <div className="space-y-3 sm:space-y-4">
              {method.methods.map((item, index) => {
                ...
                const isExpanded = expandedMethods[uniqueId];
                return (
                  <div key={index} className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      {item.image && (
                        <button
                          onClick={() => toggleMethod(uniqueId)}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[rgba(0,234,255,0.1)] border border-[rgba(0,234,255,0.3)] hover:bg-[rgba(0,234,255,0.2)] transition-all duration-300 group"
                        >
                          <ImageIcon className="w-3.5 h-3.5 text-[#00eaff] group-hover:scale-110 transition-transform" />
                          {isExpanded ? (
                            <ChevronUp className="w-3.5 h-3.5 text-[#00eaff]" />
                          ) : (
                            <ChevronDown className="w-3.5 h-3.5 text-[#00eaff]" />
                          )}
                        </button>
                      )}
                    </div>
                    ...
                    {item.image && (
                      ...
                      >
                        <div className="p-4 sm:p-6 rounded-xl bg-[rgba(0,0,0,0.4)] border-2 border-[rgba(0,234,255,0.3)] backdrop-blur-sm">
                          <img
                            src={item.image}
                            alt={item.label}
                            className="w-full h-auto rounded-lg object-contain border-2 border-[rgba(255,255,255,0.2)] shadow-2xl"
                            style={{ 
                              maxWidth: '100%', 
                              height: 'auto',
                              display: 'block',
